...
namespace SGBDBuffer
{
    ...
    {
        private const int pageLenght = 128;
        private IList<Page> _pages = new Page[10];
        private string _filePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data.txt");//Path.Combine(Environment.CurrentDirectory, "Data.txt");
            ...
            if (!PageInBuffer(page))
            {
                ...
                using (var stream = new StreamReader(File.Open(_filePath, FileMode.Open)))
                {
                    ...
                    var newPage = new Page
                    {
                        ...
                        Data = content.Split(new char[] { '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries)[page].ToCharArray()
                    };
        ...
        internal void SavePage(int page, Page info)
        {
            ...
            using (var writer = new StreamWriter(_filePath, false))
            {
                ...
                {
                    if (i == page)
                        writer.WriteLine(new string(info.Data));
                    else
